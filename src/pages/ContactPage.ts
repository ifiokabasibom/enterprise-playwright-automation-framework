...
export default class ContactPage{
    ...
    private readonly contactFullNameLableLocator = "sfa-output-name-with-hierarchy-icon-wrapper";
    ...
    async createNewContact(fname: string, lname:string) {
        ...
        await this.page.getByPlaceholder(this.firstNameTextFieldLocator).fill(lname);
    ...
    async expectContactLabelContainsFirstNameAndLastName(fname: string, lname:string) {
        await expect(this.page.locator(this.contactFullNameLableLocator)).toContainText(`${fname} ${lname}`);
